        ...
        <Box sx={{ position: 'relative' }}>
          <Box sx={{ ...styles.detailItemContainer }}>
            <Box sx={{ ...styles.subContainer }}>
              <Grid container>
                <Grid item xs={12} sx={{ display: 'flex' }}>
                  {detailsData?.payment?.payment?.paymentType === 'PurchaseOrder' ? (
                    <Grid item md={6} sm={12} sx={{ flexDirection: 'column' }}>
                      ...
                      <Typography variant="body2">
                        {t('po-number')}:{' '}
                        {
                          detailsData?.payment?.payment?.billingInfo?.purchaseOrder
                            ?.purchaseOrderNumber
                        }
                      </Typography>
                      <Typography variant="body2">
                        {t('authorizer-name')}:{' '}
                        {
                          detailsData?.payment?.payment?.billingInfo?.purchaseOrder?.customFields.find(
                            (field: { code: string }) => field.code === 'authorizer-name'
                          )?.value
                        }
                      </Typography>
                      <Typography variant="body2">
                        ...
                        {
                          detailsData?.payment?.payment?.billingInfo?.purchaseOrder?.customFields.find(
                            (field: { code: string }) => field.code === 'authorizer-phone'
                          )?.value
                        }
                      </Typography>
                      {/* <Typography variant="body2">
